...
Try ref  [<span class="ref" data-target="sec:BTmodel"></span>](#sec:BTmodel)
...
Section [<span class="ref" data-target="sec:covariates"></span>](#sec:covariates) considers variations of the standard model,
...
effects. Sections [<span class="ref" data-target="sec:ability"></span>](#sec:ability) and [<span class="ref" data-target="sec:residuals"></span>](#sec:residuals) explain how
...
residuals, whilst Section [<span class="ref" data-target="sec:model"></span>](#sec:model) notes the functions available
to aid model search. Section [<span class="ref" data-target="sec:data"></span>](#sec:data) explains in more detail how
set up data for use with the **BradleyTerry2** package,
Section [<span class="ref" data-target="sec:functions"></span>](#sec:functions) lists the functions provided by the package
and finally Section [<span class="ref" data-target="sec:finalremarks"></span>](#sec:finalremarks) comments on two directions
...
**BradleyTerry2** package as the data set `citations`:
...
users of the **lme4** package [@bate:11]. (Note that a random intercept
is the only random effect structure currently implemented in
**BradleyTerry2**.)
...
variables in the formula use [R]{.sans-serif}'s standard indexing
mechanism to include student-specific variables, e.g., `WOR`: whether or
not the student was in full-time employment, and school-specific
variables, e.g., `LAT`: whether the school was in a 'Latin' city. Thus
there are three types of variables: contest-specific (`school1`,
`school2`, `student`), subject-specific (`WOR`, `DEG`, ...) and
object-specific (`LAT`, `St.Gallen`, ...). These three types of
variables are provided in three data frames, contained in the list
object `CEMS`.
...
```{r figqvplot, echo=FALSE , fig.cap="Estimated relative abilities of baseball teams.", fig.alt="graphic without alt text", fig.show='hold', fig.align="center", out.width="67.0%"}
...
Section [<span class="ref" data-target="sec:covariates"></span>](#sec:covariates) above. That model lacks the clearly
...
```{r figresiduals, echo=FALSE , fig.cap="Lizard residuals for the simple model lizModel, plotted against throat.PC3.", fig.alt="graphic without alt text", fig.show='hold', fig.align="center", out.width="69.0%"}
...
such as the `at.home` variable seen in Section [3.3](#sec:order). If
...
The `CEMS` example in Section [3.4](#sec:CEMS) provides an illustration
...
contest, provided either directly as in Section [7.1](#sec:contest) or
via indexing as in Section [7.2](#sec:non-contest). In studies where the
...
outcome of all pairwise comparisons between the 6 schools in the study
...
illustrated in Section [<span class="ref" data-target="sec:covariates"></span>](#sec:covariates). For more general,
